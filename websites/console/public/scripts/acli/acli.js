...
(function( $ ){
            ...
            return this.each(function(){
                    ...
                    switch(key) {
                        ...
                        case 13: // enter
                            ...
                            var row = $("<div class='cli-commandRow'>")
                                .append($("<span class='cli-promptText'>").html(new Date().toLocaleTimeString() +'>'))
                ...
                function parseCommand(commandLine) {
                    ...
                    while(i<tokens.length) {
                        ...
                        if (token.indexOf('---') == 0) type = 'system'; // system param
                        else if(token.indexOf('--')==0) type = 'name'; // named param
                    ...
                    if (command.params) params = params.concat(command.params);
                    for(var i=0; i<params.length; i++) {
                        var param = params[i],
                            paramFromCommand = paramsFromCommand[param.name],
                            paramVal;
                            
                        // this is a system param which the user didn't use
                        if (param.system && !paramFromCommand)
                          continue;
                        ...
                        // this must be a required param with no value (an indexed param with no defaultValue)
                    ...
                    function getParamBy(field, name) {
                      switch (field) {
                        case 'system':
                          for (var j = 0; j < systemParams.length; j++) {
                            var param = systemParams[j];
                            if (param.name === name) {
                              return param;
                            }
                          }
                ...
                function executeCommand(command, args) {
                  ...
                  // todo: move following code to PinPanel class
                    .dblclick(function (e) {
                      if (!e.ctrlKey) return;

                      if ($(this).parent().is('.cli-pinPanel-item')) {
                  ...
                  var isLive = args.live;
                  var liveInterval; 
                  var liveCount = 0;
                  var maxLiveCount = env.env('liveIterationsCount');
                  if (isLive)
                  ...
                  execCommandInternal();
                  function execCommandInternal() {
                    if (liveCount >= maxLiveCount) {
                      clearInterval(liveInterval);
                      console.log('stopping live interval for command', command);
                      return;
                    }
                    var promise = {
                ...
                function addSystemParams() {
                  systemParams.push({
                    name: 'live',
                    description: 'makes command refresh every X seconds',
                    type: 'number',
                    defaultValue: 5
                  });
                  
                  systemParams.forEach(function(param){
                    param.system = true;
                  });
                }
                ...
                function addDefaultCommands() {
                    ...
                    var pinPanelCommand = {
                        name: 'pinPanel',
                        description: 'Manages the pinPanel panel. Using with no params will toggle the pinPanel visibility.',
                        usage: 'pinPanel [--off] [--on] [--put]',
                        example: 'pinPanel',
                        params: [
                            {
                                name: 'off',
                                description: 'turn pinPanel visibility off',
                                type: 'boolean'
                            },
                            {
                                name: 'on',
                                description: 'turn pinPanel visibility on',
                                type: 'boolean'
                            },
                            {
                                name: 'pin',
                                description: 'pin last result',
                                type: 'boolean'
                            }
                        ],
                        //hidden: true,
                        exec: function(args) {
                            ...
                            if(!args.on && !args.off && !args.pin) {
                                if ($(".cli-pinPanel").css("display")=='none')
                                    args.on = true;
                                else args.off = true;
                            }
                            ...
                            if(args.off) {
                                ...
                                pinPanel.addItemToPinPanel(item);
                            }
                        }
                    }
                ...
                function jsonToHtml(obj, ignoreJQuery) {
                    ...
                    html.find("table").each(function(){
                        var field = $(this).parent("td.metadataValue").prev("td.metadataField")
                            .click(function(){
                                ...
                                // If the value of the clicked field is an array, and this is the first click,
                                // we want the content of the array to be displayed, not just its indices
                                if (field.next("td.metadataValue").data("isArray") && !$(this).data("uncollapsed")) {
                                    $(this).data("uncollapsed", true);
                                    $(this).next("td.metadataValue").children("table").each(function(){
                                        $(this).children("tbody").children("tr").each(function(){
                                            $(this).children("td.metadataField").click();
                                        });
                                    });
                                }
                            })
                        ...
                        //  Hide all the elements which level is larger than jsonViewCollapseLevel,
                        //  unless the last non hidden elements are the indices or an array. In this case
                        //  show the values of those elements as well
                        var fieldLevel = field.parents("table").length;
                        var containingElement = field.closest("table").parent("td.metadataValue");

                        if ((fieldLevel <= level) ||
                            ( fieldLevel === level + 1 && containingElement.length !== 0 && containingElement.data("isArray"))  ) {
                            field.data("uncollapsed", true);
                        }
                        else {
                    ...
                    function _jsonToHtml(obj, ignoreJQuery) {
                           ...
                           for (var key in obj)
                           {
                               ...
                               else if (typeof obj[key] === 'number') val = obj[key];
                               else val = obj[key] || '';

                               var isArray =  (obj[key] instanceof Array );

                               $("<tr>")
                                  .append($("<td class='metadataField'>").html(key)).data("uncollapsed", false)
                                  .append($("<td class='metadataValue'>").append(val).data("isArray", isArray))
                                  .appendTo(tbl);
                           }
                ...
                function PinPanel() {
                    ...
                    var self = this;
                    var pinPanel = options.pinPanel;
                    pinPanel.find(".cli-pinPanel-title").data('on', true)
                        .click(function(){
                            if($(this).data('on')) {
                    ...
                    $(window).resize(function(){
                        pinPanel.css('height', resultsContainer.parent().height());
                    });

                    /*
                    $(".cli-pinPanel-title").hover(
                        function(){
                            $(this).parent('.cli-pinPanel').draggable().draggable( "option", "disabled", false);
                        },
                        function(){
                            $(this).parent('.cli-pinPanel').draggable( "option", "disabled", true );
                        }
                    );

                    $(".cli-pinPanel-title").click(
                        function(){
                            $(this).parent(".cli-pinPanel").find(".cli-pinPanel-item").toggle();
                        }
                    );
                    */
                    ...
                    pinPanel
                        ...
                        drop: function( event, ui ) {
                            self.addItemToPinPanel(ui.draggable);
                        }
                    });

                    this.addItemToPinPanel = function(item){
                        var conatiner = $("<div class='cli-pinPanel-item'>").appendTo(pinPanel);
                    ...
                    pinPanel.find(".cli-pinPanel-close").click(function(){
                        pinPanel.hide();
                    });
    ...
    */
    ...
    $.fn.cli.env = {
        ...
        liveIterationsCount: { type: 'number', min: 1, max: 9999, value: 9999, description: 'used to control number of ierations a "live" command refreshes itself' },
