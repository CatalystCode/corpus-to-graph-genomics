...
ECHO deploy the [azuredeploy.all-in-one.json]+[azuredeploy.all-in-one.sourcecontrol.json] 
...
call azure group deployment create -f azuredeploy.all-in-one.json -e %parameters_file% %resource_group_name% %deployment_name%-base
timeout 20

call azure group deployment create -f azuredeploy.all-in-one.sourcecontrol.json -e %parameters_file% %resource_group_name% %deployment_name%-git
